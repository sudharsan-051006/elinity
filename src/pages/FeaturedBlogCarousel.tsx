...
const featuredBlogs = blogs.slice(0, 3).map(blog => ({
  ...blog,
  tag: 'Featured',
}));
...
const FeaturedBlogCarousel = () => {
  ...
  const renderCard = (blog: typeof featuredBlogs[0], animate: boolean, animDir: 'left' | 'right' | null) => (
    ...
    >
      <div className="mx-auto w-full max-w-2xl bg-[#181848] rounded-2xl shadow-2xl border border-purple-700/40 p-6 flex flex-col md:flex-row items-center gap-6" style={{boxShadow:'0 0 32px 4px #a855f7, 0 0 0 1px #fff2'}}>
        <img src={blog.image} alt={blog.title} className="w-64 h-40 object-cover rounded-xl shadow-md" />
        <div className="flex-1 flex flex-col justify-between h-full">
          <span className="inline-block bg-gradient-to-r from-blue-500 to-purple-500 text-xs text-white px-3 py-1 rounded-full mb-2 font-semibold w-fit">{blog.tag || 'Featured'}</span>
