...
const Contact: React.FC = () => {
  ...
  // State for tracking the form submission status (success, error, and message).
  ...
  // Updates the form data state whenever a user types in an input field.
   ...
   * It prevents the default form action, validates the fields,
   * and simulates a successful submission with a timeout.
   */
  ...
  // Placeholder function for actions that are not yet implemented.
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-[#0a0a23] p-2 md:p-4 overflow-auto">
      <div className="w-full max-w-4xl mx-auto flex flex-col md:flex-row rounded-3xl overflow-hidden shadow-2xl" style={{ boxShadow: '0 8px 40px 0 rgba(168,85,247,0.25), 0 1.5px 8px 0 rgba(0,0,0,0.10)' }}>
        {/* Left Side */}
        <div className="flex-1 flex flex-col justify-center items-center px-4 md:px-8 py-10 md:py-16 relative" style={{ minHeight: 600, background: 'radial-gradient(circle at 50% 30%, #b983ff 0%, #a855f7 40%, #3a185a 80%, #181848 100%)' }}>
          ...
          <div className="relative z-10 flex flex-col items-center w-full">
            ...
            <p className="text-purple-100 mb-10 text-center max-w-xs text-base">Got any question about the Product we are here to help you 24/7.</p>
            <div className="flex flex-col gap-4 w-full max-w-xs">
              <button
                className="flex items-center gap-3 px-6 py-3 rounded-lg bg-white text-[#181848] font-bold shadow text-base focus:outline-none border-2 border-white transition hover:scale-105 hover:shadow-lg"
                onClick={() => handleAction('/chat')}
              >
                <span className="w-7 h-7 flex items-center justify-center rounded-full bg-black text-white font-bold mr-2">1</span>
                Chat With Us
              </button>
              <button
                className="flex items-center gap-3 px-6 py-3 rounded-lg bg-[#23235b] text-white font-semibold shadow text-base focus:outline-none border-2 border-[#23235b] transition hover:bg-[#2d1a4a] hover:scale-105"
                onClick={() => handleAction('/book-call')}
              >
                <span className="w-7 h-7 flex items-center justify-center rounded-full bg-[#44446b] text-white font-bold mr-2">2</span>
                Book a call
              </button>
              <button
                className="flex items-center gap-3 px-6 py-3 rounded-lg bg-[#23235b] text-white font-semibold shadow text-base focus:outline-none border-2 border-[#23235b] transition hover:bg-[#2d1a4a] hover:scale-105"
              ...
              >
                <span className="w-7 h-7 flex items-center justify-center rounded-full bg-[#44446b] text-white font-bold mr-2">3</span>
                Shoot Us with Email
              </button>
                </div>
                </div>
        ...
        {/* Right Side */}
        <div className="flex-1 bg-[#181848] flex flex-col justify-center px-4 md:px-8 py-10 md:py-16">
          <div className="max-w-md w-full mx-auto">
            <h2 className="text-3xl font-extrabold text-white mb-1">Let&apos;s Talk with Us</h2>
            <p className="text-purple-200 mb-8 text-base">Let&apos;s Talk about what we can do together.</p>
              ...
              <div className="mb-4">
                <label className="block text-sm text-purple-200 mb-1" htmlFor="message">Leave Us Message</label>
                  ...
                  placeholder="Leave Us Message"
                    rows={4}
                  className="w-full px-4 py-3 rounded-lg bg-white text-[#181848] border border-[#23235b] focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all text-base"
                  ></textarea>
                </div>
                <button
                  type="submit"
                ...
                <div className={`mt-4 p-3 rounded-lg ${formStatus.success ? 'bg-green-900 text-green-100' : 'bg-red-900 text-red-100'}`}>
                    {formStatus.message}
                  </div>
                )}
              </form>
